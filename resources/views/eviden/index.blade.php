                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <form action="{{ route('eviden.store.hadir-peserta', $jadwal->id) }}" method="POST"
                            ...
                            @csrf
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Daftar Hadir Peserta (Scan)
                            </h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Upload file .jpg/.jpeg hasil scan.
                            </p>
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <form action="{{ route('eviden.store.hadir-petugas', $jadwal->id) }}" method="POST"
                            enctype="multipart/form-data" class="p-6">
                            @csrf
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Daftar Hadir Petugas (Scan)
                            </h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Upload file .jpg/.jpeg hasil scan.
                            </p>
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <form action="{{ route('eviden.store.berita-acara', $jadwal->id) }}" method="POST"
                            enctype="multipart/form-data" class="p-6">
                            @csrf
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Berita Acara (Scan)</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Upload file .jpg/.jpeg hasil scan.
                            </p>
                            ...
                            <ul class="list-disc list-inside mt-2">
                                @if ($jadwal->peserta_seleksi_count == 0)
                                    <li><a href="{{ route('peserta.index', $jadwal->id) }}"
                                            class="font-bold underline hover:text-yellow-900 dark:hover:text-yellow-200">Data
